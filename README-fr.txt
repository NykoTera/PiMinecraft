...
5. Pour finir, lancez votre serveur : `cd zzzz` puis `sudo sh run.sh` (remplacez `zzzz` par le chemin du dossier du serveur). Une autre méthode consiste à utiliser les commandes systemd : `systemctl start yourservice.service` **(pensez à lancer le service du serveur et non celui de la sauvegarde !)**
